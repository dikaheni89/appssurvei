...
<html>
  <head>
    ...
    <title><?= $title;?></title>
    <meta name="author" content="diskomsantik@gmail.com">
	...
	<body class="bg-silver-300">
        ...
        <div>
            <div class="text-center m-b-20">
                <img class="img-circle" src="<?= base_url('template/logo.png');?>" width="110px" />
            </div>
            <form class="text-center" id="login-form" action="javascript:;" method="post">
                <h5 class="font-strong">DISKOMSANTIK KABUPATEN PANDEGLANG</h5>
		...
		$(function() {
            $('#login-form').validate({
                ...
                submitHandler: function(form) {
                      ...
                      var csrfHash = $('input[name=csrf_survei_name]').val();
                      $.ajax({
                          type: "POST",
                          url : "<?= base_url('auth/check')?>",
                          dataType:'json',
                          data: {csrf_survei_name: csrfHash, username:user, password:pass},
                          beforeSend:function(){
                                $('.box').jmspinner('large');       
                            },
