...
<div class="ibox">
    ...
    <div class="ibox-body">
		<form method="post" id="form-validation" action="javascript:;" novalidate="novalidate">
			<input type="hidden" name="id" value="<?= $video['_id'];?>" ></input>
			<div class="form-group row">
                <label class="col-sm-2 col-form-label">Title</label>
                <div class="col-sm-6">
                    <input class="form-control" type="text" name="title"  value="<?= $video['title'];?>"  placeholder="Title" autocomplete="off">
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label">Url Youtube</label>
                <div class="col-sm-6">
                    <input class="form-control" type="text" name="uri" value="<?= $video['uri'];?>"  placeholder="Url Youtube" autocomplete="off">
                </div>
            </div>
            <div class="form-group row">
    ...
    $(document).ready(function (){
        $("#form-validation").validate({
            ...
            submitHandler:function(form){
                ...
                $.ajax({
                    type: "POST",
                    url : "<?= base_url('admin/updatevideo')?>",
                    data: {<?= csrf_token() ?>:'<?= csrf_hash() ?>', id:id, title:title, uri:uri},
