            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    ...
                    <?php foreach ($input as $row) : ?>
                        <option value="<?= $row['_id'];?>"><?= $row['nm_input'];?> - Nama Survei : ( <?= $row['nm_survei'];?> )</option>
                    <?php endforeach; ?>
                    </select>
                </div>
            </div>
			<div class="form-group row">
                <label class="col-sm-2 col-form-label">Nama Kategori Subnput</label>
                <div class="col-sm-10">
                    <input class="form-control" type="text" name="nm_subinput" placeholder="Nama Kategori Subinput" autocomplete="off">
                </div>
            </div>
            <div class="form-group row">
    ...
    $(document).ready(function (){
        ...
        $("#form-validation").validate({
            ...
            submitHandler:function(form){
                ...
                $.ajax({
                    type: "POST",
                    url : "<?= base_url('admin/savekategorisubinput')?>",
                    data: {<?= csrf_token() ?>:'<?= csrf_hash() ?>',nm_subinput:nm_subinput, idinput:idinput},
